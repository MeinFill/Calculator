...
string check_tochek(string input, int& check_tochki, string& errors){ //метод ничего не менят. Только для отсчета ошибок в написании чисел
...
void up_numbers(string& buf, double*& numbers, int& kolvo_numbers) {
	int schet_toch = 0;
	...
	if (schet_toch == 1) { // если есть точка/запятая, надо создать вещественное число, отделив целую часть от дробной, а потом сложить
		...
		numbers[kolvo_numbers] = stod(ah) + (stod(al) / pow(10, al.length())); // в массив записывается число: ah менять не надо, а вот al делится на 10 в степени длины, чтобы число превратилось в дробную часть с целым 0
	}
...
void schet_viragenia(int& kolvo_deistv, string& input, double*& numbers, string*& deistv, int*& deistv_weight, int& ch_n) {
...
double calculate(string& input, int& check_tochki, int& ch_n) { // метод колькуляции и возврата ответа
	...
	if (check == 1) { //если ошибок нет, можно че-то делать
		...
		if (check_tochki == 1) {
			...
			schet_viragenia(kolvo_deistv, input, numbers, deistv, deistv_weight, ch_n);
